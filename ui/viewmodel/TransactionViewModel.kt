...
class TransactionViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun setCurrency(newCurrencyCode: String) {
        val oldCurrencyCode = _selectedCurrency.value
        if (oldCurrencyCode == newCurrencyCode) return

        viewModelScope.launch {
            val transactions = repository.allTransactions.first()
            transactions.forEach { transaction ->
                val convertedAmount = convertCurrency(transaction.amount, transaction.currencyCode, newCurrencyCode)
                repository.update(transaction.copy(amount = convertedAmount, currencyCode = newCurrencyCode))
            }
            
            // Also convert savings goal
            val convertedGoal = convertCurrency(_savingsGoal.value, oldCurrencyCode, newCurrencyCode)
            setSavingsGoal(convertedGoal)
    ...
    private fun convertCurrency(amount: Double, from: String, to: String): Double {
        ...
        // Formula: (Amount in USD) * toRate -> (Amount / fromRate) * toRate
        return (amount / fromRate) * toRate
    }

    fun addCategory(name: String, type: TransactionType) {
        if (type == TransactionType.EXPENSE) {
            val newList = (_expenseCategories.value + name).distinct().sorted()
            _expenseCategories.value = newList
            sharedPrefs.edit().putStringSet("expense_categories", newList.toSet()).apply()
        } else {
            val newList = (_incomeCategories.value + name).distinct().sorted()
